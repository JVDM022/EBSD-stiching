...
c0 = 1.01*min(cellfun(@(x) quantile(max(0,x(:)),0.01), solver.pf.allI) ./ alpha(1:length(solver.pf.allI)));
...
if c0 > 0.1 && c0 < 0.99 && ~check_option(varargin,'noGhostCorrection')
  ...
  % apply zero range method
  if check_option(varargin,'ZR')
    solver.zrm = zeroRangeMethod(solver.pf,'delta',0.01);
    solver.S3G = subGrid(solver.S3G,~solver.zrm.checkZeroRange(solver.S3G));
    solver.init;
