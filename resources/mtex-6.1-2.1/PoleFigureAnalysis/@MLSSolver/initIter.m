...
if isempty(solver.c)
  ...
  %c = ones(length(solver.S3G),1) ./ length(solver.S3G);

  %for i = 1:numPF(solver.pf)
  %  pf = interp(solver.pf.allR{i},solver.pf.allI{i},'Harmonic');
  %  values = interp(solver.pf.allR{i},solver.pf.allI{i},solver.S3G * solver.pf.allH{i}.symmetrise,'harmonic');
  %  c = c .* prod(reshape(values,numel(c),[]),2);
  %end
